...
bool checkFirebaseConnection() {
    ...
    // Check internet access AND Firebase responsiveness
    bool internetAvailable = false;
    if (wifiConnected) {
        // Attempt a small test read to Firebase (recommended method)
        if (Firebase.ready()) {
            if (Firebase.RTDB.getInt(&fbdo, "/status/pingTest")) {
                internetAvailable = true;  // Firebase responded properly
            } else {
    ...
    bool currentlyConnected = wifiConnected && internetAvailable;

    // Handle connection state changes
    if (currentlyConnected != wasFirebaseConnected) {
        if (currentlyConnected) {
            Serial.println("✅ Firebase connected");
            firebaseReconnectInterval = 2000;
    ...
    if (!currentlyConnected) {
        ...
        if (currentMillis - lastFirebaseReconnectAttempt > firebaseReconnectInterval) {
            ...
            // Optional: do a test read again to validate reconnection
            if (Firebase.ready() && Firebase.RTDB.getInt(&fbdo, "/status/pingTest")) {
                Serial.println("✅ Firebase reconnected");
                firebaseReconnectInterval = 1000;
                wasFirebaseConnected = true;
                updateDeviceStatus(true, false, false);
            } else {
                firebaseReconnectInterval = min(firebaseReconnectInterval * 2, maxFirebaseReconnectInterval);
