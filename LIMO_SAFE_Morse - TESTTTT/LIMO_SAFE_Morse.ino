...
void loop() {
    ...
    // Check if in a light sensor critical section
    if (isLightSensorCriticalSection()) {
        // Skip only high-current operations during critical sections
        return;
    }
    ...
    if (!checkWiFiConnection()) {
        ...
        if (millis() - lastFirebaseCheck >= FIREBASE_CHECK_INTERVAL) {
            ...
            if (!Firebase.ready()) {
                ...
                // Firebase-dependent operations - these can cause voltage fluctuations
                if (!isLightSensorCriticalSection()) {
