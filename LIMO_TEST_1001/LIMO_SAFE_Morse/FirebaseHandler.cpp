...
// State machine for Firebase setup
enum FirebaseSetupState {
    FB_SETUP_INIT,
    FB_SETUP_CONNECTING,
    FB_SETUP_RETRY_WAIT,
    FB_SETUP_INITIALIZE_DATA,
    FB_SETUP_UPDATE_STATUS,
    FB_SETUP_COMPLETE,
    FB_SETUP_FAILED
};

static FirebaseSetupState fbSetupState = FB_SETUP_INIT;
static unsigned long fbStateChangeTime = 0;
static int fbInitAttempts = 5;
static unsigned long fbSetupStartTime = 0;
static const unsigned long FB_MAX_ATTEMPT_TIME = 60000;  // 1 minute total attempt time
static FirebaseJson fbDeviceJson;
...
// Non-blocking Firebase setup function
bool nonBlockingFirebaseSetup() {
    unsigned long currentTime = millis();
    
    // Initialize timers on first entry
    if (fbSetupState == FB_SETUP_INIT) {
        Serial.println("🔥 Starting non-blocking Firebase setup...");
        fbSetupStartTime = currentTime;
        fbStateChangeTime = currentTime;
        fbInitAttempts = 5;
        fbSetupState = FB_SETUP_CONNECTING;
        ...
        // Set device ID
        if (!deviceId.length()) {
            Serial.println("📱 Getting device ID from MAC address");
            uint8_t mac[6];
            WiFi.macAddress(mac);
            char deviceIdBuffer[13];
            sprintf(deviceIdBuffer, "%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
    ...
    // Global timeout for the entire setup process
    if (currentTime - fbSetupStartTime > FB_MAX_ATTEMPT_TIME && fbSetupState != FB_SETUP_COMPLETE) {
        Serial.println("❌ Firebase setup timed out");
        fbSetupState = FB_SETUP_FAILED;
    }
    ...
    switch (fbSetupState) {
        ...
        case FB_SETUP_CONNECTING:
            ...
            if (isFirebaseReady()) {
                Serial.println("✅ Firebase connected successfully!");
                fbSetupState = FB_SETUP_INITIALIZE_DATA;
                fbStateChangeTime = currentTime;
            } else {
                ...
                if (fbInitAttempts <= 0) {
                    ...
                    fbSetupState = FB_SETUP_RETRY_WAIT;
                    fbStateChangeTime = currentTime;
                }
            }
            break;
            
        case FB_SETUP_RETRY_WAIT:
            // Wait 5 seconds before retrying
            if (currentTime - fbStateChangeTime >= 5000) {
                fbSetupState = FB_SETUP_CONNECTING;
                fbStateChangeTime = currentTime;
            }
            break;
            
        case FB_SETUP_INITIALIZE_DATA:
            // Initialize device data in Firebase
            {
                String path = String(DEVICE_PATH) + deviceId;
                fbDeviceJson.clear();
                ...
                if (existingData) {
                    // If we got existing data, parse it
                    fbDeviceJson = fbdo.jsonObject();
                    Serial.println("✅ Retrieved existing device data");
                }
                
                // Always ensure these basic fields exist
                fbDeviceJson.set("id", deviceId);
                fbDeviceJson.set("status/online", true);
                
                // Initialize WiFi node if it doesn't exist
                FirebaseJsonData result;
                if (!fbDeviceJson.get(result, "wifi")) {
                    FirebaseJson wifiJson;
                    wifiJson.set("connected", false);
                    fbDeviceJson.set("wifi", wifiJson);
                }
                
                // Use updateNode instead of setJSON to preserve existing data
                if (Firebase.RTDB.updateNode(&fbdo, path.c_str(), &fbDeviceJson)) {
                    Serial.println("✅ Device data initialized in Firebase");
                    fbSetupState = FB_SETUP_UPDATE_STATUS;
                    fbStateChangeTime = currentTime;
                } else {
                    Serial.println("❌ Failed to initialize device data");
                    fbSetupState = FB_SETUP_FAILED;
        ...
        case FB_SETUP_UPDATE_STATUS:
            ...
            if (updateDeviceStatus(true, false, false)) {
                ...
                fbSetupState = FB_SETUP_COMPLETE;
            } else {
                Serial.println("❌ Failed to update device status");
                fbSetupState = FB_SETUP_FAILED;
            }
        ...
        case FB_SETUP_COMPLETE:
            return true;
            
        case FB_SETUP_FAILED:
            return false;
    }
...
void resetFirebaseSetup() {
    fbSetupState = FB_SETUP_INIT;
    fbStateChangeTime = 0;
    fbInitAttempts = 5;
}